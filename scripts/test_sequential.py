...
def main():
    """Run channel 1 test - continuous flashing"""
    ...
    if not pattern:
        print("✗ No pattern for channel 1")
        ser.close()
        return 1
    
    cycle_time = sum(seg.duration for seg in pattern)
    print(f"✓ Loaded channel 1 pattern: {cycle_time:.2f}s per cycle")
    print("\n" + "="*70)
    ...
    try:
        ...
        while True:
            ...
            # Play pattern for CHANNEL 1 ONLY
            for segment in pattern:
                # Update only channel 1, keep everything else at 0
